...
lv_obj_t *create_square_main_button(lv_obj_t *parent, const char *text, const char *icon, lv_event_cb_t cb, void *user_data)
{
    ...
    lv_obj_set_user_data(ic, (void *)ICON_IDENTIFYER); // Use 100 as "TAG_ICONO"
...
void rotate_icon_by_tag(lv_obj_t * btn, int32_t angle)
{
    ...
    for(i = 0; i < lv_obj_get_child_cnt(btn); i++) 
    {
        ...
        // Find the object with our tag 100
        if(lv_obj_get_user_data(child) == (void *)ICON_IDENTIFYER) 
        {
            // 1. Force an update to get the correct dimensions
            lv_obj_update_layout(child);

            // 2. Set the pivot to the exact center of the icon
            ...
            // 3. Apply rotation (angle * 10 for tenths of a degree)
            lv_obj_set_style_transform_rotation(child, angle * 10, 0);

            // 4. Avoid shifting:
            // Because the rotation is visual, the flex layout can sometimes push it.
            // This centers the drawing relative to its original box.
            lv_obj_set_style_translate_y(child, 0, 0); 
            return; // Found and rotated
        }
    }
}
